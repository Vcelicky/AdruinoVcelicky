...
//Every iteration lasts 8 seconds. So for 10 min sleep we need ~69 iterations
#define SLEEPING_TIME 69
...
void getAnalogSample(int analogIn){
	...
	total = 0;
	sample_count = 0;
	//get 100 samples, sum them together and then make average of them
	while(sample_count < NUM_SAMPLES){
		sample = analogRead(analogIn);
		//Serial.println(sample);
		total += sample;
		sample_count++;
		delay(2);
	}
	sample = total / NUM_SAMPLES;
	sample *= Vcc;
	sample /= 1023.0;
}
...
void setup() {
  ...
  Sigfox.begin(9600);
  Serial3.begin(9600);
  //Let the serial links to establish
  delay(1000);
